...
#. Copy it to the ``secrets.py`` script in Raspberry Pi Pico 2W.
    ...
    .. note::
        ...
        If you don't have ``do_connect.py`` and ``secrets.py`` scripts in your Pico 2W, you need to create them in your Pico 2W, please refer to :ref:`iot_access` to create them.
...
#. Open the ``3_ifttt_mail.py`` file under the path of ``pico-2w-kit-main/iot``, then click **File** -> **Save as** or press ``Ctrl+Shift+S``.
