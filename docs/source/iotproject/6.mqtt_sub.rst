...
#. Upload the ``play_music.py`` file under the path of ``pico-2w-starter-kit-main/iot`` to the Raspberry Pi Pico 2W.

    .. image:: img/mqtt-A-1.png

#. Open the ``6_mqtt_subscribe_music.py`` file under the path of ``pico-2w-starter-kit-main/iot`` and click the **Run current script** button or press F5 to run it.
