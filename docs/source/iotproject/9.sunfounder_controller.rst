...
1. Upload ``ws.py`` and ``websocket_helper.py`` from the path of ``pico-2w-starter-kit-main/libs`` to the Raspberry Pi Pico 2W.
...
3. Open the ``9_sunfounder_controller.py`` under the path of ``pico-2w-starter-kit-main/iot``. Click the **Run current script** button or press F5 to run it. After successful connection, you will see the IP of Pico 2W.
